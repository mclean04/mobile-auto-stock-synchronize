...
Android chấp nhận mọi tài khoản Google đã được Firebase xác minh. Mỗi tài khoản chỉ
...
Android dùng /v1/sync/status, /v1/sync/batches (POST/GET/detail), /v1/orders (GET/detail).
...
device registrations và preview từ chối phiên Firebase mobile với admin_required.
...
Android không còn hiển thị planning/thông báo chung. Subscription FCM cũ được yêu cầu thu hồi
